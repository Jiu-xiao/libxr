...
// 构造时先建立 endpoint 对象映射，再交给上层 USB core 统一驱动 / Build the endpoint-object map first, then hand it to the shared USB core.
...
{
  // 每次重新 Init 前先重置 FIFO 账本，避免复用旧的分配状态 / Reset FIFO bookkeeping before each re-init so stale allocation state is not reused.
  ResetFifoState();
...
// 地址只在 SETUP status 之前写入硬件，保持 control-transfer 时序正确 / Write the device address only at the setup-before-status point to preserve control-transfer timing.
...
{
  // Start 是幂等的；PHY/IRQ/ROM 清理只做一次，core 寄存器则按当前状态重建 / Start is idempotent: PHY/IRQ/ROM cleanup are one-time, while core registers are rebuilt for the current state.
  if (runtime_.started)
  {
    return;
  }
...
{
  // Stop 只关中断和软断开，不回收一次性 runtime 资源 / Stop only masks interrupts and soft-disconnects; it does not release one-time runtime resources.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // 内部 PHY 生命周期按整个设备实例保留，不做反复创建销毁 / Keep the internal PHY for the lifetime of the device instance instead of repeatedly recreating it.
...
{
  // 中断句柄按实例生命周期保留，只在第一次 Start 前注册一次 / Keep the interrupt handle for the instance lifetime and register it only once before the first start.
...
{
  // 先清掉 ROM 默认 USB/CDC ACM 状态，再接管 DWC2 device core / Tear down the ROM-provided default USB/CDC ACM state before taking over the DWC2 device core.
...
{
  // 这里直接重建 DWC2 device-mode 寄存器基线：先停总中断，复位 core，再重建 FIFO/中断 mask / Rebuild the DWC2 device-mode register baseline directly: stop global interrupts, reset the core, then rebuild FIFO and interrupt masks.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // 所有 TX FIFO 尺寸寄存器先清零，再按 endpoint 配置重新分配 / Clear all TX-FIFO size registers first, then reallocate them per endpoint configuration.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // 复位后先冲刷所有 FIFO，避免旧 payload / setup 残留 / Flush all FIFOs after reset so no stale payload or setup bytes survive.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // FIFO 账本完全由当前硬件深度和 DMA 模式重新推导，不依赖旧运行态 / Recompute the FIFO bookkeeping solely from the current hardware depth and DMA mode, without relying on stale runtime state.
...
{
  // 总线复位后先清 device 级寄存器，再由 endpoint 层各自重建具体硬件配置 / After bus reset, clear the device-level register state first; each endpoint later rebuilds its own concrete hardware configuration.
  ResetFifoState();
  ResetControlState();
...
{
  // endpoint 对象本身保留，但它们的硬件绑定状态必须全部失效 / Keep the endpoint objects, but invalidate all of their hardware-bound state.
...
{
  // EP0 setup 接收窗口在 reset/setup-done 之后都要重新装填 / Reload the EP0 setup receive window after reset and after each setup-done transition.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
  ...
  if (DmaEnabled())
  {
    ...
    ASSERT(ESPUSBDetail::CacheSyncDmaBuffer(setup_packet_, SETUP_DMA_BUFFER_BYTES, false));
...
// 只跟踪 setup 请求的 data-stage 方向，供 EP0 OUT/STATUS 判定复用 / Track only the setup-request data-stage direction for later EP0 OUT/STATUS decisions.
...
{
  // 统一从 gintsts & gintmsk 取当前批次待处理中断，并在有限 guard 内排空 / Pull one interrupt batch from gintsts & gintmsk and drain it under a bounded guard.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // 总线复位路径先停所有 endpoint，再清 mask/FIFO/状态，最后把 setup 接收窗口重新挂回去 / The bus-reset path stops all endpoints first, then clears masks/FIFOs/state, and finally rearms the setup receive window.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // daint 里只保留当前方向且已使能的 endpoint 位，然后逐个转发给 endpoint 对象 / Keep only enabled endpoint bits for the current direction from daint, then forward them one by one to the endpoint objects.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // 非 DMA 模式下直接消费 grxstsp；DMA 模式下只把状态转发给 OUT endpoint / In non-DMA mode, consume grxstsp directly; in DMA mode, only forward the status to the OUT endpoint.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
  ...
  switch (status.pktsts)
  {
    ...
    case ESPUSBDetail::RX_STATUS_DATA:
    {
      ...
      {
        uint8_t sink[64] = {};
        ESPUSBDetail::ReadFifoPacket(ESPUSBDetail::GetEndpointFifo(dev, 0), sink, status.bcnt);
      }
      break;
    }
...
bool ESP32USBDevice::AllocateTxFifo(uint8_t ep_num, uint16_t packet_size, bool is_bulk,
                                    uint16_t& fifo_words)
{
  // TX FIFO 分配保持“前缀连续”布局；若中间某级无法满足，就整体拒绝，避免留下稀疏映射 / Keep TX-FIFO allocation prefix-contiguous; if any intermediate level cannot be satisfied, reject the whole request to avoid sparse mappings.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
...
{
  // RX FIFO 只允许单调扩张，不做在线收缩，避免影响已经配置的 OUT endpoint / RX FIFO is allowed to grow monotonically only; it is not shrunk online, so already configured OUT endpoints are not disturbed.
  auto* dev = reinterpret_cast<usb_dwc_dev_t*>(ESPUSBDetail::DWC2_FS_REG_BASE);
