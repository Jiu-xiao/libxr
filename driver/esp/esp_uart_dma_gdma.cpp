...
ErrorCode ESP32UartDma::InitDmaBackend()
{
  ...
  gdma_link_list_config_t rx_link_cfg = {
      .num_items = RX_DMA_DESCRIPTOR_COUNT,
      .item_alignment = 4,
      .flags = {},
  };
  ...
  const size_t rx_storage_bytes =
      AlignUp(rx_dma_chunk_size_ * RX_DMA_DESCRIPTOR_COUNT, rx_storage_alignment);
  ...
  std::array<gdma_buffer_mount_config_t, RX_DMA_DESCRIPTOR_COUNT> rx_mount = {};
  for (uint32_t i = 0; i < RX_DMA_DESCRIPTOR_COUNT; ++i)
  {
  ...
  if (gdma_link_mount_buffers(rx_dma_link_, 0, rx_mount.data(), RX_DMA_DESCRIPTOR_COUNT,
                              nullptr) != ESP_OK)
  {
    return ErrorCode::INIT_ERR;
  }
...
bool IRAM_ATTR ESP32UartDma::ResetAndRestartRxDma()
{
  ...
  for (uint32_t i = 0; i < RX_DMA_DESCRIPTOR_COUNT; ++i)
  {
    ...
    descriptor.next = &cached_descriptors[(i + 1U) % RX_DMA_DESCRIPTOR_COUNT];
  }
...
bool IRAM_ATTR ESP32UartDma::DrainCompletedDmaRxDescriptors(bool& pushed_any)
{
  ...
  bool success = true;
  for (uint32_t consumed = 0U; consumed < RX_DMA_DESCRIPTOR_COUNT; ++consumed)
  {
    auto& descriptor = rx_dma_descriptors_[rx_dma_descriptor_index_];
    ...
    rx_dma_descriptor_index_ = (rx_dma_descriptor_index_ + 1U) % RX_DMA_DESCRIPTOR_COUNT;
  }
  return success;
}
