...
class IoInFlightGuard
{
 public:
  explicit IoInFlightGuard(std::atomic<uint32_t>& counter) : counter_(counter)
  {
    counter_.fetch_add(1U, std::memory_order_acq_rel);
  }

  ~IoInFlightGuard() { counter_.fetch_sub(1U, std::memory_order_acq_rel); }

 private:
  std::atomic<uint32_t>& counter_;
};

class AtomicCounterGuard  // Remove use only once AtomicCounterGuard
{
 public:
...
ErrorCode MSPM0UART::SetConfig(UART::Configuration config)
{
  ...
  // 切模式前始终先收掉上一轮 LIN timeout 源，避免 LIN->BYTE 后遗留悬空的
  // LINC0_MATCH / Always tear down the previous LIN timeout source before
  // switching modes to avoid leaving a stale LINC0_MATCH after LIN->BYTE.
...
ErrorCode MSPM0UART::WriteFun(WritePort& port)
{
  ...
  IoInFlightGuard io_guard(uart->io_handler_inflight_);
...
{
  IoInFlightGuard io_guard(io_handler_inflight_);
