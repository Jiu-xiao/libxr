...
public:
  static ErrorCode write_fun(WritePort &port) {
    ...
    } else if (uart->tx_queue_->EmptySize() >= port.info_.data.size_) {
      uart->tx_queue_->PushBatch(port.info_.data.addr_, port.info_.data.size_);
    } else {
  ...
  STM32UART(UART_HandleTypeDef &uart_handle, RawData dma_buff_rx,
      ...
      : UART(ReadPort(rx_queue_size), WritePort(tx_queue_size)),
        ...
        tx_queue_(new BaseQueue(1, dma_buff_tx.size_)) {
  ...
  BaseQueue *rx_queue_ = nullptr, *tx_queue_ = nullptr;
