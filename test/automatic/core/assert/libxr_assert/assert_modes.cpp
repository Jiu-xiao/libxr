/**
 * @file assert_modes.cpp
 * @brief 检查四种断言开关组合下的求值、失败分发和测试断言。 /
 * Tests evaluation, failure dispatch and TEST_ASSERT in four assertion modes.
 *
 * 记录错误位置和 ISR 标记；子进程确认 TEST_ASSERT 失败会触发 SIGABRT。
 * Checks locations and ISR flags; a child verifies that TEST_ASSERT raises SIGABRT.
 */
