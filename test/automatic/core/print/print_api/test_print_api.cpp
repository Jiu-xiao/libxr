 ...
 * @brief 检查打印 API 的输出长度、缓冲区截断和错误返回。 /
 * Tests print API output lengths, buffer truncation and errors.
 *
 * 包含零容量、空指针计数、空输出和失败后的缓冲区内容。
 * Includes zero capacity, null-buffer counting, empty output and buffer state on failure.
 */
