 ...
 * @brief 编译检查：关闭浮点打印后，两种格式前端都拒绝浮点格式。 /
 * Compile check: both frontends reject float formats when float printing is disabled.
 *
 * 同时确认整数、文本和指针格式仍可用。
 * Also checks that integer, text and pointer formats remain available.
 */
