 ...
 * @brief 编译检查：两种格式前端按配置限制浮点精度。 /
 * Compile check: both frontends enforce the configured float precision limit.
 *
 * 接受零到两位精度，三位精度应返回指定错误。
 * Accepts precision zero through two and checks the error for precision three.
 */
