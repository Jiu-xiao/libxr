 ...
 * @brief 检查 MPMC 队列的基本操作和多生产者、多消费者传输。 /
 * Tests MPMC queue operations and transfers with multiple producers and consumers.
 *
 * 并发测试逐项检查数据没有丢失或重复；普通顺序读写另行检查 FIFO。
 * Concurrent cases check loss and duplication; sequential cases check FIFO order.
 */
...
namespace
{
using Queue = LibXR::MPMCQueue<uint16_t>;  ///< Default stress payload queue.
