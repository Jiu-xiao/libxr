 ...
 * @brief `LinuxSharedTopic` standard throughput/latency 子基准。 Split benchmark unit for standard throughput/latency cases.
 */
...
/**
 * @brief 执行辅助函数 `RunStandardBenchmarks`。 Execution helper function `RunStandardBenchmarks`.
 * @details 测试内容：批量执行 standard 基准集合。 Execute the batch of standard benchmark cases.
 *          测试原理：把不同 payload 的 standard case 收口在一个可复用入口里。 Group the standard payload cases behind one reusable entry.
 */
