...
class LockFreePool
{
 public:
  ...
  union alignas(LibXR::CONCURRENCY_ALIGNMENT) Slot
  {
    struct
    {
    ...
    uint8_t pad[LibXR::CONCURRENCY_ALIGNMENT];  ///< 缓存行填充 / Cache line padding
  ...
  LockFreePool(uint32_t slot_count)
      : SLOT_COUNT(slot_count),
        slots_(new (std::align_val_t(LibXR::CONCURRENCY_ALIGNMENT)) Slot[slot_count])
  {
