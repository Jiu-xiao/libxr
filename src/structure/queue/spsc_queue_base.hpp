...
class alignas(LibXR::CONCURRENCY_ALIGNMENT) SPSCQueueBase
{
 ...
 public:
  ...
  /**
   * @brief 通过一次双 span 回调生产 FIFO 前缀 / Produce a FIFO prefix in one callback.
   * @param limit 最多提供的元素数 / Maximum number of offered payload elements.
   * @param writer 接收两段存储和元素数，返回已写入的前缀元素数 /
   *        Receives two storage pointers and element counts; returns the written prefix.
   * @return 实际发布的元素数 / Number of elements published to the consumer.
   * @pre 唯一 producer；回调期间不得重入同侧操作或 Reset /
   *      Sole producer; no same-side operation or Reset during the callback.
   * @note 零 offer 不调用回调；第二段为空时指针为 nullptr。指针仅在回调期间有效。
   *       Raw 槽位步长按构造时 element_size 和 element_align 对齐计算。
   *       A zero offer skips the callback; an empty second span is null. Pointers expire
   *       on return. Raw slot stride is element_size rounded up to element_align.
   */
  template <typename Writer>
  ...
  /**
   * @brief 通过一次双 span 回调消费 FIFO 前缀 / Consume a FIFO prefix in one callback.
   * @param limit 最多提供的元素数 / Maximum number of offered payload elements.
   * @param reader 接收两段数据和元素数，返回已接受的前缀元素数 /
   *        Receives two data pointers and element counts; returns the accepted prefix.
   * @return 实际出队的元素数 / Number of elements removed from the queue.
   * @pre 唯一 consumer；回调期间不得重入同侧操作或 Reset /
   *      Sole consumer; no same-side operation or Reset during the callback.
   * @note 回调恰好调用一次，且仅当 offer 为正；返回值不得超过 offer。
   *       外部不可撤销的接收必须等于返回前缀；回调返回后不得保留指针。
   *       The callback runs once for a positive offer and returns at most that offer.
   *       Irreversible external acceptance must equal the returned prefix. Pointers
   * expire on return; raw slot stride follows the construction-time size/alignment.
   */
  template <typename Reader>
