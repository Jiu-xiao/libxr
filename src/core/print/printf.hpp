...
struct Printf::Compiled
{
 ...
 public:
  ...
  /// Ordered argument metadata used only by compile-time matching and argument packing. / 按参数顺序排列的元信息表，仅供编译期匹配和参数打包使用
  ...
  [[nodiscard]] static consteval bool Matches()
  {
    return Detail::FormatArgument::template Matches<Args...>(ArgumentList());
  }
};
}  // namespace LibXR::Print
