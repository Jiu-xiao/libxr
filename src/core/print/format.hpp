...
/// Enables octal and hexadecimal integer conversions when integers are enabled. / 在整数功能开启时启用八进制和十六进制转换
 ...
 * the old "type + flags + width + precision" payload shape.
 * 常见小场景会直接降为只携带必要立即数的窄操作码；其余情况全部回落到
 * GenericField，继续沿用旧的 “type + flags + width + precision” 载荷形状。
 */
enum class FormatOp : uint8_t
{
  ...
  GenericField = 0xF0,     ///< wide fallback payload: type, flags, width, precision / 宽回退载荷：type、flags、width、precision
 ...
 *
 * A compiled format source provides three pieces: Codes(), one contiguous
 * uint8_t byte block terminated in-band by FormatOp::End; ArgumentList(),
 * source-ordered FormatArgumentInfo metadata used only for compile-time matching
 * and stack argument packing; and Profile(), one compile-time runtime executor
 * profile.
 * 编译格式源提供三部分内容：Codes()，即一段在块内用 FormatOp::End 结束的连续
 * uint8_t 字节块；ArgumentList()，即按源串顺序排列、仅供编译期匹配与栈上参数打包
 * 使用的 FormatArgumentInfo 元信息；以及 Profile()，即一个编译期运行期执行器
 * 配置。
 */
 ...
 * immediates, or to GenericField plus the full type/flags/width/precision payload.
 ...
 * type/flags/width/precision 载荷。type 决定运行期写出语义，pack 决定参数如何
 * 打包，rule 决定编译期实参匹配规则。
 */
struct FormatField
{
