...
class Writer
{
 public:
  /**
   * @brief Runs one compiled format whose runtime pack order is provided
   *        separately from the call-site argument order.
   * @brief 执行一份已编译格式；其运行期打包顺序由独立的编译期参数顺序表提供。
   *
   * This is used by brace-style frontends that allow repeated or reordered
   * argument references such as {1} {0} while keeping the runtime bytecode
   * stream itself strictly source-ordered.
   * 该接口用于 brace 风格前端：它允许 {1} {0} 这种重复或重排参数引用，
...
class Writer::CodeReader
{
 public:
  ...
  /**
   * @brief Reads the 4-byte field payload that follows one GenericField type byte.
   * @brief 读取紧跟在 GenericField 类型字节后的 4 字节字段载荷。
   *
