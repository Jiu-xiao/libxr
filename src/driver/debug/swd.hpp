...
class Swd
{
 public:
  ...
  struct Response
  {
    ...
    uint32_t rdata = 0;        ///< 读数据（rnw=true 且 ack==OK 有效） / Read data (valid when rnw=true && ack==OK)
  ...
  static constexpr uint32_t DP_ABORT_STKCMPCLR = (1u << 1);   ///< STKCMPCLR / 清 sticky compare
  static constexpr uint32_t DP_ABORT_STKERRCLR = (1u << 2);   ///< STKERRCLR / 清 sticky error
  static constexpr uint32_t DP_ABORT_WDERRCLR = (1u << 3);    ///< WDERRCLR / 清 write data error
  static constexpr uint32_t DP_ABORT_ORUNERRCLR = (1u << 4);  ///< ORUNERRCLR / 清 overrun error

  /**
   * @brief 常用 DP CTRL/STAT 位定义 / Common DP CTRL/STAT bit definitions
   */
  static constexpr uint32_t DP_CTRLSTAT_CDBGPWRUPREQ = (1u << 28);  ///< CDBGPWRUPREQ / Debug power up request
  static constexpr uint32_t DP_CTRLSTAT_CDBGPWRUPACK = (1u << 29);  ///< CDBGPWRUPACK / Debug power up acknowledge
  static constexpr uint32_t DP_CTRLSTAT_CSYSPWRUPREQ = (1u << 30);  ///< CSYSPWRUPREQ / System power up request
  static constexpr uint32_t DP_CTRLSTAT_CSYSPWRUPACK = (1u << 31);  ///< CSYSPWRUPACK / System power up acknowledge
   ...
   */
  static constexpr uint32_t MakeSelect(uint8_t apsel, uint8_t apbanksel, uint8_t dpbanksel = 0)
  {
 ...
 public:
   ...
   *
   * 标准流程：LinesWDD）：LineReset -> 0xE79E -> LineReset -> optional idle。
  ...
  /**
   * @brief 单次 SWD transfer（不做 WAIT 重试策略） / Single SWD transfer (no WAIT retry policy)
   *
