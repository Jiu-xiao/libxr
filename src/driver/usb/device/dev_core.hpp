...
class DeviceCore
{
 ...
 private:
  DeviceComposition* composition_ = nullptr;  ///< USB 组合管理器 / USB composition manager
  ...
  struct
  {
    ...
    size_t read_request_remain = 0;         ///< 当前控制 OUT 剩余字节数 / Remaining EP0 OUT
    bool read_streaming = false;            ///< EP0 OUT 是否按包流式接收 / Stream EP0 OUT
    RawData read_remain{nullptr, 0};        ///< OUT 剩余待接收 / Remaining OUT buffer
    size_t write_request_remain = 0;        ///< 主机仍期望的 IN 字节数 / Remaining host
                                            ///< expected IN bytes
