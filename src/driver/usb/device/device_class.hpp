...
class DeviceClass : public ConfigDescriptorItem
{
 public:
   ...
   */
  size_t GetBosCapabilityCount() override { return bos_cap_num_; }
  ...
  {
    return (index < bos_cap_num_) ? bos_caps_[index] : nullptr;
  }
 ...
 private:
  void PrepareInterfaceStringIndexes(size_t interface_count);
  void ClearInterfaceStringIndexes();
  void SetInterfaceStringIndex(size_t local_interface_index, uint8_t string_index);
  ...
  uint8_t* interface_string_indexes_ =
      nullptr;                     ///< 接口字符串索引表 / Interface string index table
  size_t interface_string_count_ = 0;  ///< 接口字符串索引数量 / Interface string index count
};
