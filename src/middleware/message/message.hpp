 ...
 *       该消息系统遵循 libxr 的静态生命周期模型：初始化阶段允许分配，发布、回调和解析热路径不应产生动态分配。
...
#pragma pack(pop)
  ...
  static constexpr uint8_t PACKET_PREFIX = 0xA5;
   ...
   * @return ErrorCode
   *
   * @note 如果主题未启用缓存，DumpData() 会读取最近一次 Publish() 保存的外部地址。
   *       When cache is disabled, DumpData() reads from the external address saved by
   *       the latest Publish().
   */
 ...
 private:
  ...
  void PublishRaw(void* addr, uint32_t size, bool from_callback, bool in_isr);
  ...
  static void DispatchSubscriber(SuberBlock& block, RawData data, bool from_callback,
                                 bool in_isr);
  static void DispatchSubscribers(TopicHandle topic, RawData data, bool from_callback,
                                  bool in_isr);
};
}  // namespace LibXR
